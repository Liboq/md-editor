...
export interface ShareServiceInterface {
  /** 创建分享 */
  createShare(content: ShareContent, userId?: string): Promise<string>;
  /** 获取分享内容 */
...
function shareContentToDbFormat(
  ...
  return {
    ...
    code_theme_id: content.codeThemeId || null,
...
function dbToShareContent(dbShare: SharedArticle): ShareContent {
  return {
    ...
    codeThemeId: dbShare.code_theme_id,
 ...
 * @param userId 用户 ID（GitHub 用户 ID）
 ...
 */
export async function createShare(content: ShareContent, userId?: string): Promise<string> {
  if (!userId) {
  ...
  // 创建分享记录
  const dbData = shareContentToDbFormat(content, userId);
 ...
 * @throws {ShareError} 当获取失败时抛出
 */
export async function getUserShares(userId?: string): Promise<ShareContent[]> {
  if (!userId) {
    throw new ShareError(
      '获取分享列表需要登录',
      'UNAUTHORIZED'
